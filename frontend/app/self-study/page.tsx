    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        <div className="bg-white rounded-md border border-gray-200 p-6 mb-8">
          <form onSubmit={generate} className="space-y-4">
            ...
            <button
              ...
              className="w-full bg-indigo-800 text-white py-3 rounded-lg font-semibold hover:bg-indigo-900 disabled:opacity-50 transition"
            >
              {generating ? (
        ...
        <div>
          ...
          {items.length === 0 ? (
            <div className="text-center py-12 surface-card text-slate-500">
              <p className="font-medium text-slate-700">Your question bank is empty</p>
              <p className="text-sm mt-1">Generate your first set above.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="bg-white rounded-md border border-gray-200 p-4 flex items-center justify-between hover:border-indigo-200 transition">
                  ...
                  <Link href={`/quiz/${item.id}?source=self_study`} className="ml-4 shrink-0 bg-indigo-800 text-white px-4 py-1.5 rounded-lg text-xs font-semibold hover:bg-indigo-900">
                    Practice
                  </Link>
                </div>
              ))}
